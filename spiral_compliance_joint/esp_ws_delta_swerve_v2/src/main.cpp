...
bool create_entities()
{
  ...
  // create publisher
  name = create_name("Sanpoom/WheelModule/", String(Module_name).c_str());
...
void subscription_callback_ledcmd(const void *msgin)
{
  ...
  if (LEDmsg->data == 0)
  {
    digitalWrite(pin_WhiteLight_EN, 0);
    // Serial.println(F("LED off"));
  }
  else if (LEDmsg->data == 1)
  {
    digitalWrite(pin_WhiteLight_EN, 1);
...
void setup()
{
  ...
  // set_microros_wifi_transports("hero-wifi-router", "heroplusplus", "10.99.0.99", 8888);
  // set_microros_wifi_transports("VISTEC-GAIT_2.4G", "exvis123", "192.168.0.4", 8888);
  set_microros_wifi_transports("DualArm_2.4G", "11223344", "192.168.1.99", 8888);
...
void loop()
{
    ...
    unsigned long current_time = millis() - start_time; // Elapsed time in milliseconds
    double time_sec = current_time / 1000.0; // Convert to seconds
    float frequency = 1.0;
    float amplitude = M_PI/3;
    double y = amplitude * sin(2 * M_PI * frequency * time_sec) + M_PI/100;
    // float y = amplitude;
    ...
    // x = true;
    // x = false;
    digitalWrite(pin_WhiteLight_EN, x);
  ...
  command.goalrad[0] = constrain(y, -M_PI, M_PI); // Dir FL   //swap because difference btw motor config of real robot and sim
  // command.goalrad[1] = constrain((MotorCMDmsg->data.data[3]), -M_PI, M_PI); // Dir FR
  // command.goalrad[2] = constrain((MotorCMDmsg->data.data[5]), -M_PI, M_PI); // Dir BL
  // command.goalrad[3] = constrain((MotorCMDmsg->data.data[7]), -M_PI, M_PI); // Dir BR
  ...
  if (currentMillis - previousMillis >= interval)
  {
    ...
    if (recv_cnt > 0)
    {
      ...
      for (i = 16; i < 24; i++)
      {
        ...
        if (x == true)
        {
          DXLmsg.data.data[i] = 1;
          DEBUG_SERIAL.println(DXLmsg.data.data[i]);
        }
        else if (x == false)
        {
          DXLmsg.data.data[i] = 0;
          DEBUG_SERIAL.println(DXLmsg.data.data[i]);
        }
      }
