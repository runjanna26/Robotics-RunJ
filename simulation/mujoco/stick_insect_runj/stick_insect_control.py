...
model = mujoco.MjModel.from_xml_path("./stick_insect_robot_runj.xml")
...
with mujoco.viewer.launch_passive(model, data) as viewer:
    ...
    while viewer.is_running() : # and time.time() - start < 30
        ...
        if step_start - start < 1.0:
            ...
            # data.ctrl[joint_id_map['leg2_1']] = np.pi/2 * np.sin(2 * np.pi * frequency * step_start)
            # data.ctrl[joint_id_map['leg2_1']] = np.pi/2 * np.sin(2 * np.pi * frequency * step_start + np.pi/2)
            # data.ctrl[joint_id_map['leg2_1']] = np.pi/2 * np.sin(2 * np.pi * frequency * step_start + np.pi/2)
