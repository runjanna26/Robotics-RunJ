...
# ===========================
def l0_pmp_step(q, x_des, W_diag, K_task, Kp_diag, q_star, mu=1e-3, dt=DT):
    ...
    # ------------------------ Controller ------------------------  >>> Can be put OAIC here

    if CLAMP_VEL:
        qdot = np.clip(qdot, -VEL_LIMIT, VEL_LIMIT)

    # Integral joint velocity
    q_next = q + qdot * dt
...
def main():
    ...
    while running:
        ...
        # PMP step
        q, joints_px, x, xdot, grad_h, Nproj = l0_pmp_step(
